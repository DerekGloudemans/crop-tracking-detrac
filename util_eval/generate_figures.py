...
#result_dir = os.path.join(data_paths["tracking_output"],"quadro_skip0")
...
# with open(os.path.join(result_dir,"quadro_results.cpkl"),"wb") as f:
#     pickle.dump(agg,f)
...
for detector in agg.keys():
    ...
    for det_step in range(50):
        ...
        if det_step in agg[detector].keys():
           if detector == "quadro_skipo0" or det_step in [1,3,5,9,15,21,29,35,45]:
...
#result_dir = data_paths["tracking_output"]
result_dir = os.path.join(data_paths["tracking_output"],"ground-truth")
...
for i in range(len(track_keys)):
    ...
    for j in range(len(det_steps)):
        ...
        # get detections
        file = os.path.join(result_dir,"ground-truth_results_{}_{}.cpkl".format(id,det_step))
        try:
            with open(file,"rb") as f:
                (tracklets,metrics,time_metrics) = pickle.load(f)
        except:
